...
(function(){
  ...
  window.renderQaBoard=function(D){
    ...
    if(g('b_jira'))g('b_jira').addEventListener('click',function(){sendPrompt('Заведи незакрытые bug/task в Jira по этим АКТУАЛЬНЫМ полям из доски (лист НЕ трогай вообще — ни строки, ни статус), слинкуй с '+D.parent+', проставь в issue эпик — тот же, что у родителя, ссылки на заведённые issue дай текстом (доску НЕ показывай, ответь текстом-итогом + панель кнопок). Поля: '+JSON.stringify(state()))});
  ...
  window.renderQaBar=function(){
    ...
    if(g('qb_jira'))g('qb_jira').addEventListener('click',function(){var x=lsGet();if(!x)return need();sendPrompt('Заведи незакрытые bug/task в Jira (Betting, SportFrame) по этим АКТУАЛЬНЫМ полям из доски (лист НЕ трогай вообще — ни строки, ни статус), слинкуй с родителем из полей, проставь в issue эпик — тот же, что у родителя, ссылки на заведённые issue дай текстом (доску НЕ показывай и НЕ пересобирай — ответь текстом-итогом + панель кнопок). Поля: '+x)});
