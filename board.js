/* qa-list-to-jira board renderer (generic, обезличен).
   Хостится на публичном CDN (jsdelivr из GitHub). НЕ содержит внутренних данных:
   список разработчиков (devs), parent, list_id, template_list, owner, pull_channel —
   всё приходит из объекта DATA, который виджет передаёт в renderQaBoard(DATA).
   Использование в виджете:
     <div id="qaRoot"></div><p id="qaEmpty"></p> + кнопки b_update/b_jira/b_pull/b_create
     <script>const DATA={...}</script>
     <script src="https://cdn.jsdelivr.net/gh/USER/REPO@v1/board.js"></script>
     <script>renderQaBoard(DATA)</script>
